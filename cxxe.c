...
static void emit_native_token_range(const IR *ir, Str *out, size_t begin_tok, size_t end_tok);
...
static void emit_named_call(const IR *ir, const IRNode *call, Str *out) {
    ...
    str_ch(out, '(');
    {
        ...
        if (impossible) {
            /* Keep the CXXE call semantics best-effort without destroying
               source already emitted before this call. */
            Str fallback;
            str_init(&fallback);
            emit_unresolved_named_call(ir, call, &fallback);
            str_putn(out, fallback.data ? fallback.data : "", fallback.len);
            str_free(&fallback);
        } else {
            int emitted = 0;
            for (idx = 0; idx < pc; ++idx) {
                ...
                if (emitted) str_put(out, ", ");
                if (arg->kind == N_NAMED_ARG) {
                    size_t eq = SIZE_MAX, q;
                    for (q = (size_t)arg->first_tok; q <= (size_t)arg->last_tok && q < ir->tokens.n; ++q) {
                        if (!strcmp(ir->tokens.v[q].text, "=")) { eq = q; break; }
                    }
                    if (eq != SIZE_MAX && eq < (size_t)arg->last_tok) {
                        emit_native_token_range(ir, out, eq + 1, (size_t)arg->last_tok);
                    } else {
                        str_put(out, arg->value ? arg->value : "");
                    }
                } else {
                    emit_native_token_range(ir, out, (size_t)arg->first_tok, (size_t)arg->last_tok);
                }
                emitted = 1;
            }
        }
...
static const IRNode *find_custom_decorator_def(const IR *ir, const char *name, const char *qualified_name) {
    ...
    fake.name = (char *)name;
    fake.qualified = (char *)qualified_name;
    {
        int64_t id = -1;
        if (find_custom_decorator_use(ir, &fake, &id)) return id >= 0 ? &ir->nodes[id] : NULL;
    }
    for (i = 0; i < ir->node_count; ++i) {
        const IRNode *n = &ir->nodes[i];
...
static void emit_tokens_range(const IR *ir, Str *out, size_t a, size_t b, const char *replace_name, size_t replace_tok) {
    ...
    for (i = a; i <= b; ++i) {
        ...
        if ((strcmp(text, ".") == 0 || strcmp(text, "->") == 0) &&
            ...
            int64_t pid = -1;
            if (resolve_property_for_member_ir(ir, i, ir->tokens.v[i - 1].text,
                                                ir->tokens.v[i + 1].text, &pid)) {
                str_put(out, text);
                str_put(out, "get_");
...
static int property_assign_token(const Token *t) {
    if (!t) return 0;
    return !strcmp(t->text,"=") || !strcmp(t->text,"+=") || !strcmp(t->text,"-=") ||
           !strcmp(t->text,"*=") || !strcmp(t->text,"/=") || !strcmp(t->text,"%=") ||
           !strcmp(t->text,"&=") || !strcmp(t->text,"|=") || !strcmp(t->text,"^=") ||
           !strcmp(t->text,"<<=") || !strcmp(t->text,">>=");
}

static void emit_property_block(const IR *ir, const IRNode *block, const IRNode *prop, int is_setter, Str *out) {
    size_t a=(size_t)block->first_tok,b=(size_t)block->last_tok,i,open=SIZE_MAX;
    const char *name = prop && prop->name ? prop->name : NULL;
    for(i=a;i<=b && i<ir->tokens.n;++i) if(!strcmp(ir->tokens.v[i].text,"{")){open=i;break;}
    if(open==SIZE_MAX || b<=open) return;
    for(i=open+1;i<=b-1;++i){
        const Token *t=&ir->tokens.v[i];
        const Token *prev = i > open + 1 ? &ir->tokens.v[i-1] : NULL;
        const Token *prev2 = i > open + 2 ? &ir->tokens.v[i-2] : NULL;
        const Token *next = i + 1 < b ? &ir->tokens.v[i+1] : NULL;
        str_put(out,t->trivia);
        if(is_setter && t->kind==TK_IDENTIFIER && !strcmp(t->text,"value")) {
            int is_this_member = prev && !strcmp(prev->text,"->") && prev2 && !strcmp(prev2->text,"this");
            int is_field_write = next && (property_assign_token(next) || !strcmp(next->text,"++") || !strcmp(next->text,"--"));
            if(name && !strcmp(name,"value") && !is_this_member && is_field_write) {
                str_put(out,"this->value");
            } else {
                str_put(out,"CXXEPropertyAssignedValue");
            }
            continue;
        }
        if(name && *name && t->kind==TK_IDENTIFIER && !strcmp(t->text,name)){
            str_put(out,"this->");
            str_put(out,name);
        } else {
            str_put(out,t->text);
...
static void emit_property(const IR *ir, const IRNode *prop, Str *out) {
    ...
    if(g>=0){ str_put(out,"decltype(auto) get_"); str_put(out,name); str_put(out,"() {"); emit_property_block(ir,&ir->nodes[g],prop,0,out); str_put(out,"}\n"); }
    if(s>=0){ str_put(out,"template <typename CXXEPropertyValue>\nvoid set_"); str_put(out,name); str_put(out,"(CXXEPropertyValue&& CXXEPropertyAssignedValue) {"); emit_property_block(ir,&ir->nodes[s],prop,1,out); str_put(out,"}\n"); }
}
...
static int property_access_is_write(const IR *ir, const IRNode *node) {
    ...
    return !strcmp(ir->tokens.v[i].text,"=") || !strcmp(ir->tokens.v[i].text,"+=") || !strcmp(ir->tokens.v[i].text,"-=") ||
           ...
           !strcmp(ir->tokens.v[i].text,"++") || !strcmp(ir->tokens.v[i].text,"--");
}
...
static int node_is_property_assignment(const IR *ir, const IRNode *node, int64_t *prop_id_out, size_t *op_out, size_t *sep_out) {
    ...
    for(i=a;i<=b;++i){
        ...
        else if(!par&&!br&&!cur&&op==SIZE_MAX&&i>a&&ir->tokens.v[i-1].kind==TK_IDENTIFIER &&
                ( !strcmp(t,"=") || !strcmp(t,"+=") || !strcmp(t,"-=") || !strcmp(t,"*=") ||
                  !strcmp(t,"/=") || !strcmp(t,"%=") || !strcmp(t,"&=") || !strcmp(t,"|=") ||
                  !strcmp(t,"^=") || !strcmp(t,"<<=") || !strcmp(t,">>="))){
            size_t pn=i-1;
            if(pn>a && (!strcmp(ir->tokens.v[pn-1].text,".")||!strcmp(ir->tokens.v[pn-1].text,"->")) &&
               resolve_property_for_member_ir(ir, pn - 1, ir->tokens.v[pn - 2].text, ir->tokens.v[pn].text, &pid)){
                op=i; sep=pn-1;
            }
        }
    }
...
static void emit_property_unary(const IR *ir, const IRNode *node, Str *out) {
    size_t a=(size_t)node->first_tok,b=(size_t)node->last_tok;
    const IRNode *prop;
    const char *op;
    size_t sep=SIZE_MAX;
    int prefix;
    if (node->aux >= ir->node_count) { append_token_bytes(ir,out,a,b); return; }
    prop=&ir->nodes[node->aux];
    op=node->name ? node->name : ((a < ir->tokens.n) ? ir->tokens.v[a].text : "");
    prefix = a < ir->tokens.n && (!strcmp(ir->tokens.v[a].text,"++") || !strcmp(ir->tokens.v[a].text,"--"));
    if (prefix) {
        if (b < a + 3) { append_token_bytes(ir,out,a,b); return; }
        sep = a + 2;
        if (sep >= ir->tokens.n || (strcmp(ir->tokens.v[sep].text,".") && strcmp(ir->tokens.v[sep].text,"->"))) { append_token_bytes(ir,out,a,b); return; }
        if (!property_has_get(ir,prop) || !property_has_set(ir,prop))
            die("CXXE: property '%s' needs both getter and setter for ++/--",prop->name?prop->name:"<unnamed>");
        str_put(out,"([&]() { ");
        append_token_bytes(ir,out,a+1,sep-1);
        str_put(out,ir->tokens.v[sep].text); str_put(out,"set_"); str_put(out,prop->name); str_put(out,"(");
        append_token_bytes(ir,out,a+1,sep-1);
        str_put(out,ir->tokens.v[sep].text); str_put(out,"get_"); str_put(out,prop->name); str_put(out,"() ");
        str_put(out,!strcmp(op,"++")?"+ 1":"- 1");
        str_put(out,"); return ");
        append_token_bytes(ir,out,a+1,sep-1);
        str_put(out,ir->tokens.v[sep].text); str_put(out,"get_"); str_put(out,prop->name); str_put(out,"(); }())");
    } else {
        ...
        if (!property_has_get(ir,prop) || !property_has_set(ir,prop))
            die("CXXE: property '%s' needs both getter and setter for ++/--",prop->name?prop->name:"<unnamed>");
        str_put(out,"([&]() { auto __cxxe_old = ");
        append_token_bytes(ir,out,a,sep-1);
        str_put(out,ir->tokens.v[sep].text); str_put(out,"get_"); str_put(out,prop->name); str_put(out,"(); ");
        append_token_bytes(ir,out,a,sep-1);
        str_put(out,ir->tokens.v[sep].text); str_put(out,"set_"); str_put(out,prop->name); str_put(out,"(__cxxe_old ");
        str_put(out,!strcmp(op,"++")?"+ 1":"- 1"); str_put(out,"); return __cxxe_old; }())");
...
static void emit_property_assignment(const IR *ir, const IRNode *node, Str *out) {
    ...
    op_text=ir->tokens.v[op].text;
    append_token_bytes(ir,out,a,sep-1);
    str_put(out,ir->tokens.v[sep].text);
    str_put(out,"set_"); str_put(out,ir->nodes[pid].name?ir->nodes[pid].name:"Property"); str_ch(out,'(');
    if(strcmp(op_text,"=")!=0){
        size_t n;
        if(!property_has_get(ir,&ir->nodes[pid])) die("CXXE: property '%s' needs a getter for compound assignment '%s'",ir->nodes[pid].name?ir->nodes[pid].name:"<unnamed>",op_text);
        append_token_bytes(ir,out,a,sep-1);
        str_put(out,ir->tokens.v[sep].text); str_put(out,"get_"); str_put(out,ir->nodes[pid].name?ir->nodes[pid].name:"Property"); str_put(out,"()");
        n=strlen(op_text); if(n && op_text[n-1]=='=') n--;
        str_put(out," "); if(n) str_putn(out,op_text,n); str_put(out," ");
    }
    if(op+1<=b){
        size_t rhs_b=b;
        if(rhs_b>=a && !strcmp(ir->tokens.v[rhs_b].text,";")) rhs_b--;
        if(op+1<=rhs_b) emit_native_token_range(ir,out,op+1,rhs_b);
    }
    str_ch(out,')');
    if(b>=a && !strcmp(ir->tokens.v[b].text,";")) str_ch(out,';');
}
static void emit_function_with_custom_decorators(const IR *ir, const IRNode *fn, Str *out) {
    ...
    /* Base implementation: same signature/body, with a private unique name. */
    {
        ...
        str_put(out, " {");
        if (body_open + 1 <= (size_t)fn->last_tok) emit_native_token_range(ir, out, body_open + 1, (size_t)fn->last_tok - 1);
        str_put(out, "}\n\n");
...
static int node_needs_native_emit(const IR *ir, const IRNode *n) {
    ...
    if (n->kind == N_UNARY_EXPR && n->name && (!strcmp(n->name,"++") || !strcmp(n->name,"--"))) return 1;
    ...
    if (n->kind == N_ASSIGN_EXPR) return node_is_property_assignment(ir, n, NULL, NULL, NULL) ||
        (n->name && (!strcmp(n->name,"++") || !strcmp(n->name,"--")));
...
static void emit_native_token_range(const IR *ir, Str *out, size_t begin_tok, size_t end_tok) {
    size_t i, native_count = 0, cursor_tok = begin_tok;
    size_t *native_ids = NULL;
    if (begin_tok > end_tok || begin_tok >= ir->tokens.n) return;
    if (end_tok >= ir->tokens.n) end_tok = ir->tokens.n - 1;

    for (i = 0; i < ir->node_count; ++i) {
        const IRNode *n = &ir->nodes[i];
        if (!node_needs_native_emit(ir, n)) continue;
        if (n->flags & NF_EXTERNAL_DECL) continue;
        if (n->first_tok < begin_tok || n->last_tok > end_tok || n->first_tok > n->last_tok) continue;
        native_ids = (size_t *)xrealloc(native_ids, (native_count + 1) * sizeof(*native_ids));
        native_ids[native_count++] = i;
    }

    for (i = 1; i < native_count; ++i) {
        size_t key = native_ids[i], j = i;
        while (j > 0) {
            size_t prev = native_ids[j - 1];
            if (ir->nodes[prev].first_tok < ir->nodes[key].first_tok ||
                (ir->nodes[prev].first_tok == ir->nodes[key].first_tok &&
                 ir->nodes[prev].last_tok <= ir->nodes[key].last_tok)) break;
            native_ids[j] = prev;
            --j;
        }
        native_ids[j] = key;
    ...
    for (i = 0; i < native_count; ++i) {
        const IRNode *n = &ir->nodes[native_ids[i]];
        size_t first_tok = (size_t)n->first_tok;
        size_t last_tok = (size_t)n->last_tok;
        uint64_t bs;
        if (first_tok < cursor_tok) continue;
        if (first_tok > end_tok || last_tok > end_tok) continue;
        if (first_tok > cursor_tok) {
            uint64_t gb = ir->tokens.v[cursor_tok].byte_start;
            uint64_t ge = ir->tokens.v[first_tok - 1].byte_end;
            if (ge > gb) emit_source_range_with_properties(ir, out, gb, ge);
        }
        bs = ir->tokens.v[first_tok].byte_start;
        if (n->kind == N_DECORATOR || n->kind == N_CUSTOM_DECORATOR) {
            /* CXXE-only syntax disappears. */
        } else if (n->kind == N_MATCH_STMT) {
            emit_match_statement(ir, n, out);
        } else if (n->kind == N_PROPERTY) {
            emit_property(ir, n, out);
        } else if (n->kind == N_UNARY_EXPR) {
            emit_property_unary(ir, n, out);
        ...
        } else if (node_is_factory_runtime_call(n)) {
            emit_factory_runtime(ir, n, out);
        } else {
            emit_named_call(ir, n, out);
        }
        (void)bs;
        cursor_tok = last_tok + 1;
    }
    if (cursor_tok <= end_tok) {
        uint64_t gb = ir->tokens.v[cursor_tok].byte_start;
        uint64_t ge = ir->tokens.v[end_tok].byte_end;
        if (ge > gb) emit_source_range_with_properties(ir, out, gb, ge);
    }
    free(native_ids);
}
...
static void emit_cpp(const IR *ir, const char *path) {
    Str out;
    size_t cursor = 0, i, native_count = 0;
    size_t *native_ids = NULL;
    int runtime_features = class_has_runtime_features(ir);
    str_init(&out);

    /* Native nodes are created by several parser passes. In particular,
       property-assignment nodes are intentionally discovered before generic
       expression nodes, so IR creation order is not source order. The source
       cursor below can only be correct when native emission is ordered by the
       covered token range. */
    for (i = 0; i < ir->node_count; ++i) {
        if (!node_needs_native_emit(ir, &ir->nodes[i])) continue;
        if (ir->nodes[i].flags & NF_EXTERNAL_DECL) continue;
        if (ir->nodes[i].first_tok >= ir->tokens.n || ir->nodes[i].last_tok >= ir->tokens.n ||
            ir->nodes[i].first_tok > ir->nodes[i].last_tok) continue;
        native_ids = (size_t *)xrealloc(native_ids, (native_count + 1) * sizeof(*native_ids));
        native_ids[native_count++] = i;
    }
    for (i = 1; i < native_count; ++i) {
    ...
    for (i = 0; i < native_count; ++i) {
        const IRNode *n = &ir->nodes[native_ids[i]];
        {
            ...
            else if (n->kind == N_UNARY_EXPR) { emit_property_unary(ir, n, &out); }
            else if ((n->kind == N_FUNCTION || n->kind == N_METHOD) && function_has_custom_decorator(ir, n)) {
                emit_function_with_custom_decorators(ir, n, &out);
            }
            else if (n->kind == N_PROPERTY_ACCESS) {
